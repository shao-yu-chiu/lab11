...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbre = new MyDBHelper(this).getWritableDatabase();

        btn_insert.setOnClickListener(view ->{
            if(ed_book.length()<1 || ed_price.length()<1)
                Toast.makeText(MainActivity.this,"欄位請留空",Toast.LENGTH_SHORT).show();
            else{
                try {
                    dbre.execSQL("Insert INTO myTABLE(book,price) values(?,?)",new Object[] {ed_book.getText().toString(),ed_price.getText().toString()});
                    Toast.makeText(MainActivity.this,"成功報名",Toast.LENGTH_SHORT).show();
                    ed_book.setText("");
                    ed_price.setText("");
                }catch (Exception e){
                    e.printStackTrace();
        ...
        btn_update.setOnClickListener(view->{
            if(ed_book.length()<1 || ed_price.length()<1)
                ...
                try {
                    dbre.execSQL("UPDATE myTable SET price ="
                            + ed_price.getText().toString()
                            + " WHERE book LIKE'"
                            +ed_book.getText().toString()+"'");
                    Toast.makeText(MainActivity.this,"更新書名",Toast.LENGTH_SHORT).show();
                    ed_book.setText("");
                    ed_price.setText("");
                }catch (Exception e){
                    e.printStackTrace();
        ...
        btn_delete.setOnClickListener(view -> {
            if(ed_book.length()<1 || ed_price.length()<1)
                ...
                try {
                    dbre.execSQL("DELETE FROM myTable WHERE book LIKE'" + ed_book.getText().toString() +"'");
        ...
        btn_query.setOnClickListener(view -> {
            Cursor c;
            if(ed_book.length()<1)
                c=dbre.rawQuery("SELECT * FROM myTable",null);
            else{
               c=dbre.rawQuery("SELECT * FROM myTable WHERE book LIKE '" + ed_book.getText().toString()+"'",null);
            }
            c.moveToFirst();
            items.clear();
